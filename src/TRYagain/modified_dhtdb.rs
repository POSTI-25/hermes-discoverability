...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut routing_table = rt::RoutingTable::new(*swarm.local_peer_id());
...
fn handle_input_line(
    ...
    match args.next() {
        ...
        Some("PUT_USERNAME") => {
            if let (Some(username), Some(peer_id_str)) = (args.next(), args.next()) {
                if let Ok(peer_id) = PeerId::from_str(peer_id_str) {
                    let record = kad::Record {
                        key: kad::RecordKey::new(&username),
                        value: peer_id.to_string().into_bytes(),
                        publisher: None,
                        expires: None,
                    };
                    if let Err(e) = kademlia.put_record(record, kad::Quorum::One) {
                        eprintln!("Failed to put username record: {:?}", e);
                    }
                } else {
                    eprintln!("Invalid PeerId provided for username.");
                }
            } else {
                eprintln!("Usage: PUT_USERNAME <username> <peer_id>");
            }
        }
        Some("PRINT_RT") => {
            routing_table.print();
        ...
        _ => {
            eprintln!("expected GET, GET_PROVIDERS, PUT, PUT_PROVIDER, PUT_USERNAME, or PRINT_RT");
        }
    }
}
